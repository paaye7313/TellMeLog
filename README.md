...
### 5. 대용량 파싱 성능 최적화 — 1분 → 1초

**문제:** 10MB 로그 파일(~12만 줄) 파싱 시 UI가 1분 이상 완전히 멈추는 현상이 발생했습니다.

**원인 분석 및 단계별 해결:**

**1차 개선 (60초 → 30초): UI 이벤트 큐 과부하 해소**
- 원인: `entryCallback`이 줄마다 `QMetaObject::invokeMethod`를 호출 → UI 이벤트 큐에 12만 개의 `insertRow` 요청이 쌓여 스크롤·버튼 등 다른 이벤트가 처리되지 못함
- 해결: 500줄 또는 100ms 단위 배치 전송으로 변경, `setUpdatesEnabled(false/true)`로 배치 삽입 중 리렌더링 차단
- 추가: `finished` 슬롯에서 `applyFilters()` 중복 호출 제거 (entryCallback이 이미 테이블을 완성한 상태)

**2차 개선 (30초 → 1초): 후처리 병목 제거**
- 원인 1: UI 스레드(`finished` 슬롯)에서 12만 건 `sourceFile` 세팅, `QDateTime::fromString()` 12만 번 호출, 대용량 데이터 복사가 한꺼번에 실행됨
- 해결 1: `ParseResult` 구조체 도입 → 후처리 전체를 `QtConcurrent` 백그라운드로 이동
- 원인 2: 날짜 범위(`minDt/maxDt`) 계산을 위해 전체 엔트리를 순회하며 `QDateTime::fromString()` 12만 번 호출
- 해결 2: 로그는 시간순 기록이므로 첫 번째·마지막 parsed 엔트리만 확인 → 호출 횟수 12만 번 → 2번

---

## 📊 성능 측정

**파싱 소요 시간 (10MB, ~12만 줄 기준)**

| 단계 | 소요 시간 | 개선 내용 |
|---|---|---|
| 초기 구현 | 60초+ | 매 줄마다 UI 스레드에 렌더링 요청 |
| 1차 개선 | ~30초 | 배치 방식 전환, 중복 재렌더링 제거 |
| 2차 개선 | ~1초 | 후처리 백그라운드 이동, `QDateTime` 호출 12만 → 2번 |
...
- [x] 대용량 파싱 성능 최적화 (60초 → 1초, QtConcurrent 배치 처리)
- [x] 파싱 결과 캐시 — 파일 전환 시 재파싱 없이 즉시 표시